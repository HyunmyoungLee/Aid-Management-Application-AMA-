...
namespace sict{
	...
	std::ostream& NonPerishable::write(std::ostream& os, bool linear) const{
		if(linear == false){
			os << std::setw(max_sku_length +1) << std::left << sku <<"|";
			...
			os << "Price: " << price << endl;
			os << "either of:" << endl;
			os << "        Price after tax:" << NonPerishable::cost() <<endl;
			os << "        N/A" << endl;
			os << "Quantity On Hand: " << CurrentQTY << endl;
			os << "Quantity Needed: " << NeededQTY << endl;
		}
		return os;
